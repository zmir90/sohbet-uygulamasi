// === server.js - NİHAİ v22 - SQL KESİN SON DÜZELTME ===
...
// --- Tabloları oluştur/kontrol et (SIFIRDAN YAZILDI) ---
(async () => {
    let client;
    try {
        ...
        console.log("'users' tablosu OK.");
         ...
         // user_sessions tablosu (TAM ve DOĞRU)
        await client.query(`
         ...
         // Primary key'i ayrı ekleyelim (varsa hata vermez - DOĞRU SQL)
         await client.query(`
            DO $$ BEGIN
                IF NOT EXISTS (
                    SELECT 1 FROM pg_constraint
                    WHERE conrelid = 'user_sessions'::regclass
                    AND conname = 'user_sessions_pkey'
                ) THEN
